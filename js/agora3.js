document.addEventListener("DOMContentLoaded", function() {
    ...
    function blockElements() {
        const iframeDocument = myFrame.contentDocument || myFrame.contentWindow.document;
        const removeElements = () => {
            iframeDocument.querySelectorAll("tr").forEach((element) => element.remove());
            iframeDocument.querySelectorAll("div.swiper-box.animated01[data-v-4c1da8ff][data-v-9f489d03]").forEach((element) => element.remove());
            iframeDocument.querySelectorAll("nav.flex.aligns-center.navbar").forEach((element) => element.remove());
        };
